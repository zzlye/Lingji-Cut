...
from backend.api.automation import _prepare_auto_resume_for_transient_failure  # noqa: E402
...
class AutomationJobTests(unittest.TestCase):
    ...
    def test_build_gemini_transcriber_uses_frontend_audio_split_settings(self):
        ...
        self.assertEqual(transcriber.settings["audio_prompt"], "")
        self.assertEqual(transcriber.settings["retry_count"], 1)
    ...
    def test_transient_gemini_failure_prepares_auto_resume(self):
        """Gemini 429/限流这类临时错误应自动进入断点续跑等待，而不是只停下等手动继续"""
        job = AutomationJobRecord(
            id="auto-gemini-auto-resume",
            source_url="https://youtube.com/watch?v=test",
            status="failed",
            progress=58,
            current_step="流程失败",
            error_message="字幕处理失败",
            params=json.dumps({
                "url": "https://youtube.com/watch?v=test",
                "subtitle_recognition_mode": "gemini_align",
                "gemini_audio_retry_count": 2,
            }, ensure_ascii=False),
            stages=json.dumps([
                {"key": "download", "status": "completed", "progress": 100, "task_id": 1, "output_path": "D:/download.mp4", "error_message": None},
                {"key": "effects", "status": "completed", "progress": 100, "task_id": 2, "output_path": "D:/effects.mp4", "error_message": None},
                {"key": "subtitle", "status": "failed", "progress": 55, "task_id": 3, "output_path": None, "error_message": "Gemini 失败"},
                {"key": "export", "status": "pending", "progress": 0, "task_id": None, "output_path": None, "error_message": None},
            ], ensure_ascii=False),
        )
        db = FakeDb([job])

        delay = _prepare_auto_resume_for_transient_failure(
            db,
            job,
            RuntimeError("字幕处理失败: Gemini 音频识别失败: HTTP 429 Resource has been exhausted (rate limited)"),
        )
        stages = {stage["key"]: stage for stage in json.loads(job.stages)}
        params = json.loads(job.params)
        ...
        self.assertEqual(job.status, "pending")
        self.assertIn("自动继续", job.current_step)
        self.assertEqual(stages["download"]["status"], "completed")
        self.assertEqual(stages["effects"]["status"], "completed")
        self.assertEqual(stages["subtitle"]["status"], "pending")
        self.assertEqual(params["_auto_resume"]["gemini_audio_retry_count"], 1)
    ...
    def test_rerun_overrides_refresh_current_api_profiles(self):
        ...
        params = _merge_rerun_overrides(
            ...
            AutomationRerunOverrideRequest(
                ...
                original_volume=0.8,
                voice_window_size=6,
                voice_window_chars=480,
                voice_window_max_ms=10000,
                voice_window_gap_ms=500,
        ...
        self.assertEqual(saved["voice_window_chars"], 480)
        self.assertEqual(saved["voice_window_max_ms"], 10000)
        self.assertEqual(saved["voice_window_gap_ms"], 500)
    ...
    def test_automation_voice_uses_continuous_window_timeline_by_default(self):
        ...
        class CapturingVoiceEngine:
            ...
            async def generate_timed_voice_track(self, *_args, **_kwargs):
                """默认不应退回串行逐句接口"""
                raise AssertionError("默认不应调用串行逐句接口")

            async def generate_voice(self, *_args, **_kwargs):
                """字幕时间轴可用时不应退回整段配音"""
                raise AssertionError("时间轴配音不应调用整段接口")

        with tempfile.TemporaryDirectory(prefix="automation_voice_window_") as temp_dir:
            downloaded_path = os.path.join(temp_dir, "downloaded.mp4")
            with open(downloaded_path, "wb") as file:
                file.write(b"video")
            fake_downloader = FakeAutomationDownloader(downloaded_path)
            fake_processor = FakeAutomationProcessor(temp_dir)
            voice_engine = CapturingVoiceEngine()
            video = VideoSource(id=1, platform="youtube", video_id="voice-window-default", url="https://example.test/video", title="默认窗口配音")
            voice_profile = VoiceProviderProfile(
                id=6,
                name="配音",
                provider_type="openai_tts",
            ...
            with (
                ...
                response = _run_automation_sync(
                    AutomationRunRequest(
                        ...
                        voice_window_size=5,
                        voice_window_chars=900,
                        voice_window_max_ms=9000,
                        voice_window_gap_ms=600,
                        voice_concurrency=3,
                    ),
                    FakeDb([]),
                )
