...
- `/automation/start`、`/automation/jobs`、`/automation/jobs/{id}`、`/automation/jobs/{id}/events`、`/automation/jobs/{id}/retry` 可创建、列表展示、实时推送、查询和重试后台自动化任务。
...
- 一键流程已沉到后端后台任务编排，并支持 SSE 实时进度和任务重试；后续还需要断点恢复和更完整的批量任务调度。
